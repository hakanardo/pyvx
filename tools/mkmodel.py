funcs = [' vx_context vxCreateContext(void)',
...
for f in funcs:
    n = re.search(r'^\s*[^\s]+\s*\*?\s*([^\(\s]+)\s*\(', f).group(1)
    pyn = re.subn(r'([a-z0-9])([A-Z])', r'\1_\2', n[2:])[0].lower()
    ps  = pyn.split('_')
    if ps[0] in ('query', 'release'): continue
    if ps[0] == 'set' and ps[-1] == 'attribute': continue
    args = [a.split(' ')[-1].replace('*', '') for a in f[:-1].split(',')]
    print """
@api('%s')
@capi('%s')
def %s(%s):
    raise NotImplementedError
""" % (n[2:], f.strip(), pyn, ', '.join(args))