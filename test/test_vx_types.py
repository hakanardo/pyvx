...
class TestVxTypes(object):
    def test_imagepatch_addressing_t(self):
        a = vx.imagepatch_addressing_t()
        assert a.dim_x == 0
        a = vx.imagepatch_addressing_t(dim_x=1, dim_y=2, scale_x=3, scale_y=4, stride_x=5,
    ...
    def test_float_consts(self):
        assert vx.SCALE_PYRAMID_HALF == 0.5

    def test_perf_t(self):
        p = vx.perf_t()
        assert p.tmp == 0
        p = vx.perf_t(tmp=1, beg=2, end=3, sum=4, avg=5, min=6, num=7, max=8)
        assert p.tmp == 1
        assert p.beg == 2
        assert p.end == 3
        assert p.sum == 4
    ...
    def test_border_mode_t(self):
        ...
        assert b.mode == vx.BORDER_MODE_CONSTANT
        assert b.constant_value == 42

    def test_keypoint_t(self):
        k = vx.keypoint_t(x=1, y=2, strength=3.5, scale=4.5, orientation=5.5, tracking_status=6, error=7.5)
        assert  k.x == 1
        assert  k.y == 2
    ...
    def test_coordinates2d_t(self):
        c = vx.coordinates2d_t(1, 2)
        assert c.x == 1
        assert c.y == 2

    def test_coordinates3d_t(self):
        c = vx.coordinates3d_t(1, 2, 3)
        assert c.x == 1
        assert c.y == 2
        assert c.z == 3
